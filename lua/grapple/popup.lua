...
function M.open_tags(scope)
    ...
    local lines = tags.serialize_tags(scope)
    ...
    local function close()
        ...
        tags.resolve_tags(scope, remaining_lines)
    end
    ...
    local function select()
        ...
        local tag_key = tags.parse_tag(line)
        if tag_key == nil then
            return
        end
...
function M.open_scopes()
    local buffer = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_buf_set_option(buffer, "bufhidden", "wipe")

    local lines = tags.serialize_scopes()
    vim.api.nvim_buf_set_lines(buffer, 0, -1, false, lines)
    ...
    local function close()
        ...
        tags.resolve_scopes(remaining_lines)
    end
