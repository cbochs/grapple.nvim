...
local resolvers = {
    basic = require("grapple.scope").static("scope", { persist = false }),
    other = require("grapple.scope").static("other_scope", { persist = false }),
}
...
describe("tags", function()
    before_each(function()
        ...
        require("grapple.tags").reset(resolvers.basic)
        require("grapple.tags").reset(resolvers.other)
    end)

    describe("#tag", function()
        it("creates a tag for a given buffer", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_one })
            assert.not_nil(require("grapple.tags").find(resolvers.basic, { buffer = buffer_one }))
        end)

        it("does not tag buffers that have excluded filetypes", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_two })
            assert.is_nil(require("grapple.tags").find(resolvers.basic, { buffer = buffer_two }))
        end)

        it("does not create duplicate tags for the same buffer", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_unnamed })
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_unnamed })
            assert.equals(1, require("grapple.tags").count(resolvers.basic))
        end)

        it("does not create duplicate tags for the same file path", function()
            require("grapple.tags").tag(resolvers.basic, { file_path = file_two })
            require("grapple.tags").tag(resolvers.basic, { file_path = file_two })
            assert.equals(1, require("grapple.tags").count(resolvers.basic))
        end)
    end)
    ...
    describe("#untag", function()
        it("removes a tag on a given buffer", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_one })
            require("grapple.tags").untag(resolvers.basic, { buffer = buffer_one })
            assert.is_nil(require("grapple.tags").find(resolvers.basic, { buffer = buffer_one }))
        end)
    end)

    describe("#update", function()
        it("updates a tags cursor location", function()
            local tag = require("grapple.tags").tag(resolvers.basic, { buffer = buffer_one })

            require("grapple.tags").update(resolvers.basic, tag, { 10, 11 })
            tag = require("grapple.tags").find(resolvers.basic, { buffer = buffer_one })
    ...
    describe("#select", function()
        ...
        it("selects an unopen file path", function()
            local tag = require("grapple.tags").tag(resolvers.basic, { file_path = file_two })
            require("grapple.tags").update(resolvers.basic, tag, cursor_two)

            tag = require("grapple.tags").find(resolvers.basic, { file_path = file_two })
            require("grapple.tags").select(tag)
    ...
    describe("#find", function()
        ...
        it("finds a tag for a given file path", function()
            require("grapple.tags").tag(resolvers.basic, { file_path = file_two })
            assert.not_nil(require("grapple.tags").find(resolvers.basic, { file_path = file_two }))
        end)

        it("finds a tag for a given tag key", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_one, key = "one" })
            assert.not_nil(require("grapple.tags").find(resolvers.basic, { key = "one" }))
        end)

        it("does not return a tag when the tag does not exist", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_one, key = "one" })
            assert.is_nil(require("grapple.tags").find(resolvers.basic, { key = "blah" }))
        end)
    end)

    describe("#count", function()
        it("returns the number of tags in a tag scope", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = 0 })
            assert.equals(1, require("grapple.tags").count(resolvers.basic))
        end)
    end)
    ...
    describe("#quickfix", function()
        it("populates the quickfix list for a given scope", function()
            require("grapple.tags").tag(resolvers.basic, { buffer = buffer_one })
            require("grapple.tags").quickfix(resolvers.basic)
        ...
        it("does not populate the quickfix list with other scope tags", function()
            require("grapple.tags").tag(resolvers.other, { buffer = 0 })
            require("grapple.tags").quickfix(resolvers.basic)
