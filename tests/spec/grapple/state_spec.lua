...
describe("state", function()
    ...
    describe("#with_keys", function()
        it("returns the state items with their keys embedded", function()
            local full_items = require("grapple.state").with_keys(resolvers.project_one)
            assert.equals(1, full_items[1].key)
            assert.equals("keyed_tag", full_items[2].key)
        end)
    end)
